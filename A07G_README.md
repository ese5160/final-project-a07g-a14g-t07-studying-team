...
7. 1. User types a character. TODO
   1. The character is transmitted through USART and get into the register `latestRx`. 
   2. An interrupt is called, triggering the callback function `usart_read_callback()`. 
   3. The callback function checks if the buffer `cbufRx` is full or not. 
   4. If `cbufRx` is not full, the data stored in the register `latestRx` are put into `cbufRx`, and restart the read job. 
